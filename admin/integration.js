(() => {
  ...
  function renderCards(){
    ...
    $('#integration-connectors').innerHTML=rows.map(x=>`<article class="integration-card"><div class="integration-card-head"><h3>${esc(x.name)}</h3><span class="integration-pill ${esc(x.status)}">${labels[x.status]||esc(x.status)}</span></div><p class="integration-purpose">${esc(x.purpose)}</p><div class="integration-facts"><div><small>반영 방식</small><b>${modes[x.sync_mode]||esc(x.sync_mode)}</b></div><div><small>데이터 방향</small><b>${directions[x.data_direction]||esc(x.data_direction)}</b></div></div><div class="integration-next"><small>다음 행동</small><p>${esc(x.next_action||'별도 작업 없음')}</p></div>${x.admin_url?`<a href="${esc(x.admin_url)}" target="_blank" rel="noopener noreferrer">원본 열기 ↗</a>`:''}</article>`).join('')||'<p class="integration-empty">이 상태에 해당하는 시스템이 없어요.</p>';
  }
