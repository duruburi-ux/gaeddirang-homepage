...
Deploy with `verify_jwt: false` so browser preflight and modern asymmetric Supabase access tokens reach the function. This does not make the endpoint public: the function itself requires a bearer token, validates the current user with Supabase Auth, checks the existing `is_admin()` RPC, and compares the current email with the admin allowlist on every request, including cache hits. CORS is limited to the canonical website and its www variant. Only GET and OPTIONS are accepted.
