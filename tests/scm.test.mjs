...
function fixture(){return {
 ...
 books:{values:[['SKU','현재고','정가','저자','ISBN','비고'],['TEST-B',8,12000,'테스트 작가','9790000000000','외부위탁 · 분기 정산']]},
...
test('mapping preserves stock and adds admin bibliography and operations context',()=>{const d=buildData(fixture());assert.equal(d.inventory[0].stock,8);assert.equal(d.inventory[1].stock,1);assert.equal(d.inventory[0].cost,null);assert.equal(d.inventory[0].author,'테스트 작가');assert.equal(d.inventory[0].isbn,'9790000000000');assert.match(d.inventory[0].note,/분기 정산/);assert.equal(d.blind[0].price,15000);assert.equal(d.blind[0].check,'')});
...
test('browser scripts parse; SCM admin exposes bibliography without embedded inventory or secret',()=>{const h=readFileSync(new URL('../admin/index.html',import.meta.url),'utf8'),s=readFileSync(new URL('../admin/scm.js',import.meta.url),'utf8');for(const m of h.matchAll(/<script>([\s\S]*?)<\/script>/g))new vm.Script(m[1]);new vm.Script(s);for(const tab of ['home','work','integration','orders','apps','members','cohorts','inquiries','alerts','scm','operations'])assert.ok(h.includes(`id="tab-${tab}"`));assert.match(h,/상품·작가·재고 소유·운영 메모/);assert.match(h,/상품·작가/);assert.match(s,/도서 현재고/);assert.match(s,/운영·정산 메모/);assert.ok(!h.includes('app-data'));assert.ok(!h.includes('BEGIN PRIVATE KEY'));assert.ok(h.includes("const TABS=['home','work','integration','operations','scm','orders','apps','members','cohorts','inquiries','alerts']"))});
