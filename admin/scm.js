...
window.SCM=(()=>{
  ...
  function detail(sku){const x=data?.inventory.find(x=>x.sku===sku);if(!x)return;const pairs=[['작가',x.author||'미확인'],['분류',x.category],['판매가',money(x.price)],['원가 / 정산 기준',money(x.cost)],['현재고',x.stock??'미확인'],['상품SKU마스터',x.skuStock??'미확인'],['재고 소유',x.owner],['SKU',x.sku],['ISBN / 바코드',x.isbn||x.barcode||'미확인']];$('#scm-detail-body').innerHTML=`<p class="scm-eyebrow">PRODUCT / READ ONLY</p><h2>${esc(x.name)}</h2><dl>${pairs.map(([k,v])=>`<div><dt>${esc(k)}</dt><dd>${esc(v)}</dd></div>`).join('')}</dl>${x.note?`<p class="scm-notice"><b>운영·정산 메모</b><br>${esc(x.note)}</p>`:''}<p class="scm-notice">장부값이며 실물 일치나 지급 확정을 뜻하지 않습니다. 원가 미입력은 0원이 아닙니다.</p>`;$('#scm-detail').showModal()}
  ...
  function render(){if(!data)return;
    ...
    const rows=data.inventory.filter(x=>(!q||norm([x.name,x.author,x.owner,x.sku,x.isbn,x.barcode].join(' ')).includes(q))&&(!category||x.category===category)&&(!f||(f==='attention'?attention(x):f==='zero'?x.stock===0:x.stock>0&&!x.status.includes('반품'))));
    rows.sort((a,b)=>sort==='name'?a.name.localeCompare(b.name,'ko'):(a[sort]??Infinity)-(b[sort]??Infinity));
    $('#scm-rows').innerHTML=rows.length?rows.map(x=>`<tr><td><button class="scm-name" data-scm-product="${esc(x.sku)}">${esc(x.name)}</button><span class="scm-sku">${esc(x.author||'작가 미확인')} · ${esc(x.sku)}</span></td><td>${esc(x.category)}</td><td>${esc(x.owner)}</td><td>${money(x.price)}</td><td>${x.stock??'—'}</td><td><span class="scm-tag ${attention(x)?'bad':''}">${esc(x.status)}</span>${x.stock!==x.skuStock?'<span class="scm-sku">마스터 불일치</span>':''}</td></tr>`).join(''):'<tr><td colspan="6" class="scm-empty">해당 상품이 없습니다.</td></tr>';
    const owners=[...new Set(rows.map(x=>x.owner))].sort().map(owner=>`${owner} ${rows.filter(x=>x.owner===owner).length}종`).join(' · ');
    $('#scm-result').textContent=`${rows.length}종 표시 / 전체 ${data.inventory.length}종 · ${owners||'소유 구분 미확인'} · 1~3개 보유는 재주문 권고가 아닙니다.`;
  ...
  function apply(next){
    ...
    const bookStock=data.inventory.filter(x=>x.kind==='도서').reduce((sum,x)=>sum+(Number.isInteger(x.stock)&&x.stock>0?x.stock:0),0);
    const cards=[['관리 상품',data.inventory.length,'inventory'],['도서 현재고',bookStock,'inventory'],['확인 필요',data.inventory.filter(attention).length,'attention'],['장부 경고',data.actions.length,'checks']];
