...
test('표 머리말을 이름으로 오인하지 않고 파일명에서 이름과 직함을 복구한다', () => {
  const result = model.parseProfile(`
기간
독립출판을 비롯한 다양한 콘텐츠를 제작하는 콘텐츠 크리에이터로 활동 중
경력
2020.01~2022.01 방송 프로그램 구성작가
저서
2025.06 에세이 <감정 기록의 시작>
강연 / 행사 / 모임
2026.01 감정 표현 교실 강연 화성 다원이음터도서관
`, ['[이력서] 감정기록가 이다솜.pdf'.normalize('NFD')]);
  assert.equal(result.name, '이다솜');
  assert.equal(result.headline, '감정 기록가');
});

test('PDF 표가 날짜 열 다음 활동 열 순서로 풀려도 경력 행을 다시 짝짓는다', () => {
  const result = model.parseProfile(`
이다솜
경력
기간
...
`, ['이다솜 이력서.pdf']);
  ...
  assert.match(result.careers, /2018\.09~2019\.03 tvN/);
  assert.doesNotMatch(result.careers, /^2020\.01~2022\.01$/m);
});

test('PDF 강연 표에서 날짜 위아래로 갈라진 프로그램명과 기관을 한 행으로 묶는다', () => {
  const result = model.parseProfile(`
이다솜
경력
2020.01~2022.01 KBS 방송작가
저서
2025.06 에세이 <감정 기록의 시작>
강연 / 행사 / 모임
감정을 다채롭게 표현하는
2026.01 화성 다원이음터도서관 / 겨울독서교실
<내 마음을 톡! 감정 표현 교실> 강연 (총 5회차 진행)
사진을 보고 글로 기록하는
2025.11~12 화성 양감초등학교 & 양감작은도서관
<포토북 글쓰기> 강연 (총 6회차 진행)
제 2회 인문문화축제
2025.11 문체부/한국문화예술위원회
<모든 감정 도감> 감정 기록 강연/체험
`);
  assert.match(result.lectures, /2026\.01 감정을 다채롭게 표현하는.*다원이음터도서관.*내 마음을 톡/);
  assert.match(result.lectures, /2025\.11~12 사진을 보고 글로 기록하는.*양감초등학교.*포토북 글쓰기/);
